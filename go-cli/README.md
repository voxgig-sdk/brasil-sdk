...
boru-driven command-line client **and** interactive REPL for the Brasil
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/brasil-cli)
make build
...
./brasil-cli list bank
./brasil-cli load 1 bank            # {id:1} shorthand
./brasil-cli load '{id:1}' bank       # explicit match map
./brasil-cli list cep

# 5. Override the API base URL for a single call
BRASIL_BASE=https://api.example.com ./brasil-cli list bank
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/brasil-cli
   ```
...
```sh
./brasil-cli load 1 bank          # scalar shorthand for {id:1}
./brasil-cli load '{id:1}' bank     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `brasil>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bank`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/brasil-cli`. |
...
(`./brasil-cli <expr>`) and interactively (the REPL), and expressions compose the

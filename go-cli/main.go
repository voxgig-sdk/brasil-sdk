// Command Brasil-cli is a CLI + REPL for the Brasil SDK,
...
const entitiesHelp = "bank cep cnpj ddd feriado fipe_marca fipe_preco municipio ufn ufn2"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BRASIL_APIKEY (api key), BRASIL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BRASIL_APIKEY carries the API key and
	// BRASIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRASIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRASIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBrasilSDK(opts)

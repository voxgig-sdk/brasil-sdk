...
The Python SDK for the Brasil API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Bank(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Bank(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Brasil(None).load(
    {"id": "test01"}, None
)

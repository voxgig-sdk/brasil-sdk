...
class BrasilTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BrasilTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BrasilTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BrasilTestRunner.getenv("BRASIL_TEST_LIVE")
        override = BrasilTestRunner.getenv("BRASIL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BrasilTestRunner.getenv(key)
        ...
        explain = BrasilTestRunner.getenv("BRASIL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRASIL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BrasilTestRunner._test_control is not None:
            return BrasilTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BrasilTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BrasilTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BrasilTestRunner.load_test_control()
...
def load_env_local():
    BrasilTestRunner.load_env_local()


def env_override(m):
    return BrasilTestRunner.env_override(m)


def entity_data(v):
    return BrasilTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BrasilTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BrasilTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BrasilTestRunner.load_test_control()


def live_client_options():
    return BrasilTestRunner.live_client_options()


def live_delay_ms():
    return BrasilTestRunner.live_delay_ms()

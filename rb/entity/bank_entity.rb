...
class BankEntity
  ...
  def make
    opts = @_entopts.dup
    BankEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BrasilHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Bank, Hash] the current Bank data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BrasilHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Bank fields)
  def match_get
  ...
  # Load a single Bank.
  #
  # @param reqmatch [BankLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Bank.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Bank, Hash] the loaded Bank; raises BrasilError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BrasilHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BankListMatch, Hash, nil] match filter (any subset of
  #   Bank fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Bank>, Array] the matching Bank items; raises BrasilError on failure

...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = BrasilOmni.make_runner(TEST_JSON_FILE, BrasilSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : BrasilOmni.errify(err)
    end

...
class UfnDirectTest < Minitest::Test
  def test_direct_list_ufn
    setup = ufn_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-ufn", setup[:live] ? "live" : "unit")
    if _should_skip

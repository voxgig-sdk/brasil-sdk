...
class UfnEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Ufn(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Ufn(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BRASIL_TEST_UFN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    ufn_ref01_data = nil
    if ufn_ref01_data_raw.length > 0
...
def ufn_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "ufn", "UfnTestData.json")
  ...
  entid_env_raw = ENV["BRASIL_TEST_UFN_ENTID"]

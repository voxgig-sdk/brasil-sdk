...
class FipeMarcaEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = fipe_marca_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "fipe_marca." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BRASIL_TEST_FIPE_MARCA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    fipe_marca_ref01_data = nil
    if fipe_marca_ref01_data_raw.length > 0
...
def fipe_marca_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "fipe_marca", "FipeMarcaTestData.json")
  ...
  entid_env_raw = ENV["BRASIL_TEST_FIPE_MARCA_ENTID"]

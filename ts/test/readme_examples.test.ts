...
//     (\`new BrasilSDK(...)\` / \`BrasilSDK.test(...)\` become
//     \`BrasilSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new BrasilSDK(...)\` and any
// \`BrasilSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BrasilSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BrasilSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

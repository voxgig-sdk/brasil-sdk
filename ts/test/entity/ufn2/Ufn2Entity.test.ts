...
describe('Ufn2Entity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'ufn2.' + op, live)) return
    }
    ...
    let ufn2_ref01_data = Object.values(setup.data.existing.ufn2)[0] as any

    // LOAD
    const ufn2_ref01_ent = client.Ufn2()
    const ufn2_ref01_match_dt0: any = {}
    ufn2_ref01_match_dt0.id = ufn2_ref01_data.id
    const ufn2_ref01_data_dt0 = await ufn2_ref01_ent.load(ufn2_ref01_match_dt0)
    assert(ufn2_ref01_data_dt0.id === ufn2_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/ufn2/Ufn2TestData.json')
  ...
  const idmapEnvVal = process.env['BRASIL_TEST_UFN__ENTID']

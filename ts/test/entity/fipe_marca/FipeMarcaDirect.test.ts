...
describe('FipeMarcaDirect', async () => {
  ...
  test('direct-load-fipe_marca', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-fipe_marca', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BRASIL_TEST_FIPE_MARCA_ENTID']

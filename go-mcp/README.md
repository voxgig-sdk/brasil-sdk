...
[MCP](https://modelcontextprotocol.io) server exposing the Brasil SDK as
two agent tools — `brasil_list` and `brasil_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/brasil-mcp)
make build

# 2. Provide credentials via the environment
export BRASIL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user brasil \
  -- /absolute/path/to/brasil-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./brasil-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// brasil_list: first page of records
{ "entity": "bank" }
{ "entity": "bank", "query": { } }

// brasil_load: one record by id
{ "entity": "bank", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/brasil-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user brasil \
     -- "$PWD"/dist/*/brasil-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `brasil_list` and `brasil_load` tools now appear
   in new sessions. Ask the agent to *"list bank using brasil"*
   and it calls `brasil_list` with `{"entity":"bank"}`.
...
```sh
export BRASIL_APIKEY=sk_live_xxx            # API key
export BRASIL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `brasil_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `brasil_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRASIL_APIKEY` | API key sent with every request. |
| `BRASIL_BASE` | Optional override of the API base URL. |
...
```sh
./brasil-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"brasil_load","arguments":{"entity":"bank","query":{"id":1}}}}'
```

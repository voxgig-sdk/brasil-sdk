...
MCP server exposing the Brasil SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `brasil_list` | `entity`, optional `query` map | First page of records as JSON |
| `brasil_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `brasil_list` and
`brasil_load` tools become available in new sessions.

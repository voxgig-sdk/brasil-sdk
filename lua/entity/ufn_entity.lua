...
local UfnEntity = {}
UfnEntity.__index = UfnEntity


function UfnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UfnEntity)
  self._name = "ufn"
  self._client = client
...
function UfnEntity:get_name()
  return self._name
end


function UfnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UfnEntity.new(self._client, opts)
end
...
function UfnEntity:mark_deleted()
  self._deleted = true
end


function UfnEntity:deleted()
  return true == self._deleted
end


function UfnEntity:data_set(args)
  if args ~= nil then
...
function UfnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UfnEntity:match_set(args)
  if args ~= nil then
...
function UfnEntity:stream(action, args, callopts)
...
---@param reqmatch UfnLoadMatch
---@param ctrl? table
---@return Ufn
---@return string? err
function UfnEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function UfnEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local FipeMarcaEntity = {}
FipeMarcaEntity.__index = FipeMarcaEntity


function FipeMarcaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FipeMarcaEntity)
  self._name = "fipe_marca"
  self._client = client
...
function FipeMarcaEntity:get_name()
  return self._name
end


function FipeMarcaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FipeMarcaEntity.new(self._client, opts)
end
...
function FipeMarcaEntity:mark_deleted()
  self._deleted = true
end


function FipeMarcaEntity:deleted()
  return true == self._deleted
end


function FipeMarcaEntity:data_set(args)
  if args ~= nil then
...
function FipeMarcaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FipeMarcaEntity:match_set(args)
  if args ~= nil then
...
function FipeMarcaEntity:match_get()
...
function FipeMarcaEntity:stream(action, args, callopts)
...
---@param reqmatch FipeMarcaLoadMatch
---@param ctrl? table
---@return FipeMarca
---@return string? err
function FipeMarcaEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FipeMarcaEntity:_run_op(ctx, post_done)
  local utility = self._utility

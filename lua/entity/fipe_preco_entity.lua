...
local FipePrecoEntity = {}
FipePrecoEntity.__index = FipePrecoEntity


function FipePrecoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FipePrecoEntity)
  self._name = "fipe_preco"
  self._client = client
...
function FipePrecoEntity:get_name()
  return self._name
end


function FipePrecoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FipePrecoEntity.new(self._client, opts)
end
...
function FipePrecoEntity:mark_deleted()
  self._deleted = true
end


function FipePrecoEntity:deleted()
  return true == self._deleted
end


function FipePrecoEntity:data_set(args)
  if args ~= nil then
...
function FipePrecoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FipePrecoEntity:match_set(args)
  if args ~= nil then
...
function FipePrecoEntity:match_get()
...
function FipePrecoEntity:stream(action, args, callopts)
...
---@param reqmatch FipePrecoLoadMatch
---@param ctrl? table
---@return FipePreco
---@return string? err
function FipePrecoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FipePrecoEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local CnpjEntity = {}
CnpjEntity.__index = CnpjEntity


function CnpjEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CnpjEntity)
  self._name = "cnpj"
  self._client = client
...
function CnpjEntity:get_name()
  return self._name
end


function CnpjEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CnpjEntity.new(self._client, opts)
end
...
function CnpjEntity:mark_deleted()
  self._deleted = true
end


function CnpjEntity:deleted()
  return true == self._deleted
end


function CnpjEntity:data_set(args)
  if args ~= nil then
...
function CnpjEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CnpjEntity:match_set(args)
  if args ~= nil then
...
function CnpjEntity:stream(action, args, callopts)
...
---@param reqmatch CnpjLoadMatch
---@param ctrl? table
---@return Cnpj
---@return string? err
function CnpjEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CnpjEntity:_run_op(ctx, post_done)
  local utility = self._utility

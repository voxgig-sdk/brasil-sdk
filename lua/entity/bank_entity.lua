...
local BankEntity = {}
BankEntity.__index = BankEntity


function BankEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BankEntity)
  self._name = "bank"
  self._client = client
...
function BankEntity:get_name()
  return self._name
end


function BankEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BankEntity.new(self._client, opts)
end
...
function BankEntity:mark_deleted()
  self._deleted = true
end


function BankEntity:deleted()
  return true == self._deleted
end


function BankEntity:data_set(args)
  if args ~= nil then
...
function BankEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BankEntity:match_set(args)
  if args ~= nil then
...
function BankEntity:stream(action, args, callopts)
...
---@param reqmatch BankLoadMatch
---@param ctrl? table
---@return Bank
---@return string? err
function BankEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch BankListMatch
---@param ctrl? table
---@return Bank[]
---@return string? err
function BankEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function BankEntity:_run_op(ctx, post_done)
  local utility = self._utility

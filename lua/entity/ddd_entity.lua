...
local DddEntity = {}
DddEntity.__index = DddEntity


function DddEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DddEntity)
  self._name = "ddd"
  self._client = client
...
function DddEntity:get_name()
  return self._name
end


function DddEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DddEntity.new(self._client, opts)
end
...
function DddEntity:mark_deleted()
  self._deleted = true
end


function DddEntity:deleted()
  return true == self._deleted
end


function DddEntity:data_set(args)
  if args ~= nil then
...
function DddEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DddEntity:match_set(args)
  if args ~= nil then
...
function DddEntity:stream(action, args, callopts)
...
---@param reqmatch DddLoadMatch
---@param ctrl? table
---@return Ddd
---@return string? err
function DddEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DddEntity:_run_op(ctx, post_done)
  local utility = self._utility

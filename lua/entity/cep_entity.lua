...
local CepEntity = {}
CepEntity.__index = CepEntity


function CepEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CepEntity)
  self._name = "cep"
  self._client = client
...
function CepEntity:get_name()
  return self._name
end


function CepEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CepEntity.new(self._client, opts)
end
...
function CepEntity:mark_deleted()
  self._deleted = true
end


function CepEntity:deleted()
  return true == self._deleted
end


function CepEntity:data_set(args)
  if args ~= nil then
...
function CepEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CepEntity:match_set(args)
  if args ~= nil then
...
function CepEntity:stream(action, args, callopts)
...
---@param reqmatch CepLoadMatch
---@param ctrl? table
---@return Cep
---@return string? err
function CepEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CepEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
local Ufn2Entity = {}
Ufn2Entity.__index = Ufn2Entity


function Ufn2Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Ufn2Entity)
  self._name = "ufn2"
  self._client = client
...
function Ufn2Entity:get_name()
  return self._name
end


function Ufn2Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Ufn2Entity.new(self._client, opts)
end


function Ufn2Entity:data_set(args)
  if args ~= nil then
...
function Ufn2Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Ufn2Entity:match_set(args)
  if args ~= nil then
...
function Ufn2Entity:stream(action, args, callopts)
...
---@param reqmatch Ufn2LoadMatch
---@param ctrl? table
---@return Ufn2
---@return string? err
function Ufn2Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
function Ufn2Entity:_run_op(ctx, post_done)
  local utility = self._utility

...
The Lua SDK for the Brasil API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Bank(nil):list(nil, nil)
...
```lua
local result, err = client:Bank(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Brasil(nil):load(
  { id = "test01" }, nil
)

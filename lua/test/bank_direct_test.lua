...
describe("BankDirect", function()
  it("should direct-list-bank", function()
    local setup = bank_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-bank", setup.live and "live" or "unit")
  ...
  it("should direct-load-bank", function()
    local setup = bank_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-bank", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function bank_direct_setup(mockres)

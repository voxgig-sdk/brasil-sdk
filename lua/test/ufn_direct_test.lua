...
describe("UfnDirect", function()
  it("should direct-list-ufn", function()
    local setup = ufn_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-ufn", setup.live and "live" or "unit")
  ...
  it("should direct-load-ufn", function()
    local setup = ufn_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-ufn", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function ufn_direct_setup(mockres)

...
// Brasil SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class BrasilMakeError
{
    public static function call(?BrasilContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BrasilContext([], null);
        }
        $op = $ctx->op ?? new BrasilOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BrasilResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BrasilError) ? $err->msg : (string)$err;
        $msg = "BrasilSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new BrasilError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof BrasilError) {

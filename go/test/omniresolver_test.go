...
import (
	...
	omni "github.com/voxgig-sdk/brasil-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BrasilError); is {
				out := map[string]any{
					"name":    "BrasilError",
...
func omniCtx(arg any, client *sdk.BrasilSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BrasilSDK); is {
			client = live

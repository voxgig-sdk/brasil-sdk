...
{
    private static function utility(): BrasilUtility
    {
        return new BrasilUtility();
    }
    ...
    private static function ctx(array $over = []): BrasilContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BrasilContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BrasilOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BrasilError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BrasilOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BrasilMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BrasilOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BrasilMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BrasilOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BrasilMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BrasilSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BrasilMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BrasilResponse([]),
            'result' => new BrasilResult([]),
        ]);
        [, $err] = BrasilMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BrasilSpec(['step' => 's']),
            'response' => new BrasilResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BrasilResult([]),
        ]);
        [, $err] = BrasilMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BrasilSpec(['step' => 's']),
            'response' => new BrasilResponse(self::resp(200, ['v' => 1])),
            'result' => new BrasilResult([]),
        ]);
        [, $err] = BrasilMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BrasilSpec(['step' => 's']),
            'response' => new BrasilResponse(self::resp(200, ['v' => 2])),
            'result' => new BrasilResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BrasilMakeResponse::call($ctx);
    ...
    {
        $preset = new BrasilResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BrasilSpec([]),
            'response' => new BrasilResponse([]),
            'result' => new BrasilResult([]),
        ...
        [$got, $err] = BrasilMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BrasilResult([])]);
        [, $err] = BrasilMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BrasilSpec([]), 'result' => null]);
        [, $err] = BrasilMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BrasilSpec(['step' => 's']),
            'result' => new BrasilResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BrasilOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BrasilMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BrasilSpec(['step' => 's']),
            'result' => new BrasilResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BrasilOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BrasilMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BrasilResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BrasilSpec([]),
            'result' => new BrasilResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BrasilMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BrasilMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BrasilError('boom', 'boom');
        $utility->fetcher = function (BrasilContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BrasilSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BrasilMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BrasilContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BrasilMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BrasilResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BrasilContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BrasilSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BrasilMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BrasilResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BrasilSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BrasilMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BrasilMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BrasilMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BrasilResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BrasilDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BrasilResult(['ok' => false])]);
        $this->expectException(BrasilError::class);
        BrasilDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BrasilResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BrasilMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BrasilBaseFeature();
        $b = new BrasilBaseFeature();
        BrasilFeatureAdd::call($ctx, $a);
        BrasilFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BrasilBaseFeature
    {
        $f = new BrasilBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BrasilFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BrasilContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BrasilSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BrasilPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BrasilResponse(['headers' => null]),
            'result' => new BrasilResult([]),
        ]);
        BrasilResultHeaders::call($ctx);

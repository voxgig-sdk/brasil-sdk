...
//    only - a typed BrasilContext there would make every ctx
...
final class BrasilOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BrasilContext($clean, null);
    ...
    public static function err_from_map($m): ?BrasilError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BrasilError(is_string($code) ? $code : '', $msg);
    }

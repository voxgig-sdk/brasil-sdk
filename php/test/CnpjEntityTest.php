...
class CnpjEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = BrasilSDK::test(null, null);
        $ent = $testsdk->Cnpj(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = cnpj_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "cnpj." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BRASIL_TEST_CNPJ_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $cnpj_ref01_data = null;
        if (count($cnpj_ref01_data_raw) > 0) {
...
function cnpj_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/cnpj/CnpjTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["cnpj01", "cnpj02", "cnpj03", "v101", "v102", "v103"] as $k) {
    ...
    $entid_env_raw = getenv("BRASIL_TEST_CNPJ_ENTID");

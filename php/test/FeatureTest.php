...
class FtCtrl extends BrasilControl
{
...
class FtClient
{
    ...
    private BrasilUtility $utility;

    public function __construct(BrasilUtility $utility, array $options)
    {
    ...
    public function get_utility(): BrasilUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BrasilContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BrasilContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BrasilConfig::make_feature($name);
    ...
    private static function buildUrl(BrasilSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BrasilError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BrasilResponse($fetched);
        }
    ...
    private function fail(BrasilContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BrasilContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
